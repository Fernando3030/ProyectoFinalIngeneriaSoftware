...
public class SubirArchivoProfesor {
	...
	public SubirArchivoProfesor() {
		frame = new JFrame("Subir Archivos Propios");
		...
		lblTitulo= new JLabel("Archivos Propios");
		...
		panelCentro = new JPanel();
		 lblAutorLibro = new JLabel("Autor :");
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(libroBuscado != null)
            	{
					...
					try {
				        ...
				        resultado=sentencias.executeQuery("SELECT * FROM archivosprofesor WHERE cod_arch_prof='"+libroBuscado+"'");
						...
						{	
				        	...
				        	txtURL.setText(resultado.getString(5));
				        	txtNombreArchivo.setText(resultado.getString(3));
				        	txtAutor.setText(resultado.getString(7));
				...
				if(libroBuscado != null)
            	{
	            	 ...
	            	 try {
	            		 resultado=sentencias.executeQuery("SELECT * FROM archivosprofesor WHERE cod_arch_prof='"+libroBuscado+"'");
	            		  while(resultado.next())
							{	
	            			  String fichero2= resultado.getString(5);
		        	         ...
		        	         else
		        	         {
	            			  ...
	            			  sentencias.executeUpdate("DELETE FROM archivosprofesor WHERE cod_arch_prof='"+libroBuscado+"'");
	...
	private void initialize() {
		...
		lblAutorLibro.setBounds(10, 97, 119, 28);
				 ...
				 if(banderaModificar==false)
	                {
	              	   ...
	              	   if(banderaNombre && banderaAutor && banderaUrl)
	                     {
	              		   /** creamos la carpeta Libros donde se guardaran nuestros archivos .pdf**/
	              		    File directorio=new File("Libros Propios");
	                        directorio.mkdir(); 
							...
							try {
	                         ...
	                         try {
	                        	 ...
	                        	 sentencias.executeUpdate("INSERT INTO archivosprofesor VALUES ('"+codigo+"',"+"'"+date.format(now)+"',"+"'"+nombre+"',"+"'"+resultado.getString("cod_admin")+"',"+"'"+destino+"',"+"'"+nombreRealArchivo+"','"+autor+"')");
	                          }
	                          catch (SQLException ex) {
	                ...
	                else
	                {
	              	   ...
	              	   if(banderaNombre && banderaAutor && banderaUrl)
	                    {
                         ...
                         SimpleDateFormat date = new SimpleDateFormat("yyyy-MM-dd");
		                    	 ...
		                    	 try {
		                             sentencias.executeUpdate("UPDATE archivosprofesor SET fecha_profe='"+date.format(now)+"',"+"nombrelibro_prof='"+nombre+"',"+"autorlibro_prof='"+autor+"'"+" WHERE cod_arch_prof='"+libroBuscado+"'");
		                             libroBuscado=""; 
		                    	 }
		                           catch (SQLException ex) {
